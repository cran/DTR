...
DTR.Logrank.test <- function(fdata # A complete data frame representing the data for two-stage randomization designs
                               # fdata = data frame {X, TR, R, Z, U, delta}
) {
  ...
  names(TEST) <- c("H0", "(standardized) test statistic", "p")  
  return(TEST)    
